...
**版本 0.1.7；需要 Core >=0.2.15 且 <0.3.0。** 静音 `0/0` 的 `start-window → display reminder ACK → dashboard confirm → display idle ACK` 已完成生产真板 E2E，状态为 `VERIFIED`；物理磁铁 `opening/close-ignore/away` 序列仍需人工操作，状态为 `PLANNED`。仓库测试另覆盖内存协议、状态机、绑定和 effect 失败边界。

本应用是软件-only Application 插件：不访问 COM3、不启动/停止 Edge、不烧录、不修改现网配置。它只通过公开 SDK 请求已绑定的 Capability。

## WebUI 贡献

`plugin.yaml` 声明 `ui.apiVersion: 1`，Core 会为已安装实例生成导航“霍尔药盒”和独立路由 `/apps/hall-pillbox`。停用后入口仍保留，页面会明确显示当前未启用。页面由通用 section 组成：实例状态、来自 `window` 记录的指标、来自 `jobs.scheduled` 的计划、`window` 记录时间线、手动操作和配置表单；`start-window`、`confirm-window`、`status` 作为手动动作展示，开盖确认仍以霍尔 `away`/兼容 `open` 事件为第一事实源。表单覆盖时区、药格标识、每日计划和提醒音档位；显示参数仍保留在高级详情。
